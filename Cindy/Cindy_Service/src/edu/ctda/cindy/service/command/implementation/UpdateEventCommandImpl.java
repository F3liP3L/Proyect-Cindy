...
import edu.ctda.cindy.service.bussines.event.implementation.UpdateEventUseCaseImpl;
import edu.ctda.cindy.service.command.UpdateEventCommand;

public class UpdateEventCommandImpl implements UpdateEventCommand {
	private final DAOFactory factory = DAOFactory.getDAOFactory(DAOFactoryType.POSTGRESQL);
	private final UpdateEventUseCase useCase = new UpdateEventUseCaseImpl(factory);
	...
	public void execute(EventDTO event) {
		try {
			...
			throw ServiceCustomException.wrapException(Messages.UpdateEventUseCaseImpl.BUSSINES_EVENT_EXIST_IN_DATE, exception);
			}
		} catch (Exception exception) {
			factory.cancelTransaction();
			throw ServiceCustomException.createBussinesException(Messages.UpdateEventUseCaseImpl.BUSSINES_EVENT_UPDATE_UNEXPECTED, exception);
		} finally {
